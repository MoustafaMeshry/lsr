...
class LayoutPretrainingModel(BaseHeadSynthesisModel):

  def __init__(self,
               config: Dict,
               order: Enum=layers.NHWC):
    """
    Initializes a few-shot neural talking heads model.
    ...
    Args:
      ...
      - order: Enum, one of {layers.NHWC or NCHW} to specify the channel format
         of image tensors.
    """
    super(LayoutPretrainingModel, self).__init__(config, order)
    ## Build networks.
    ...
    if self.config.separate_layout_encoder:
      ...
      self._layout_encoder = networks.build_encoder(
          ...
          order=self.order)
    # Place holder for other train/finetune networks and optimizers.
    self._vgg_loss = None
  ...
  def create_optimizers(
    ...
    This function should be called within `strategy.scope()` if using
      tf.distribute.Strategy. It initilizes the following components:
      - self._g_layout_optimizer: This optimizer handles the segmentation loss
          optimization.
    ...
    self._g_layout_optimizer = tf.keras.optimizers.Adam(
      learning_rate=g_lr_callback,  # self.g_lr,
  ...
  def compute_losses(
    ...
    if self.config.multi_task_training:
      ...
      # VGG loss.
      per_example_loss_g_vgg = per_example_loss_g_l1 = 0
  ...
  def compute_latents(
    ...
    """Computes style latents given the input to the style encoder.

    Args:
      - encoder_inputs: 4D tensor, the input the encoder network.
    ...
    if num_few_shots == 0:
      ...
      encoder_inputs = tf.reshape(encoder_inputs, [-1, dims[2], dims[3], dims[4]])
      z_layout, _ = self._layout_encoder(encoder_inputs, training=training)
    ...
    if self.config.separate_layout_encoder:
      ...
      z_style, z_layout = tf.split(z_layout, num_or_size_splits=2, axis=-1)
    assert z_layout is not None
  ...
  def train(
    ...
    if train_g_step:
      ...
      g_layout_vars_all = g_layout_vars + e_layout_vars
      train_g_op = model_utils.get_train_op(
          tape, self._g_layout_optimizer, loss_g, g_layout_vars_all,
    ...
    summaries = self._add_summaries(
        ...
        visualize_rgb=self.config.multi_task_training)
  ...
  def get_networks(self) -> Dict[Text, Union[
    ...
    if self.config.separate_layout_encoder:
      ...
      networks_dict['encoder'] = self._layout_encoder
